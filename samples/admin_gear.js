...
si.innerHTML="";
//el.stopPropagation();
//wraptable.style.borederColor="inherit";
}
...
function on_checkPartner(ev){
	//alert(vl.crowd);
if(!vl.crowd){console.log("No vl.crowd?");return;}
if(ev.target.checked){

//on_tabi_click(ev);
//class_adi(tabi[1],"active");
if(vl.crowd.disabled){
vl.crowd.disabled=false;
vl.crowd.style.display="block";
}
}else{
vl.crowd.disabled=true;	
vl.crowd.style.display="none";
}
//ev.stopPropagation();	
}
function on_inv(el){
...
class_adi(el.parentNode,"invalid");	
